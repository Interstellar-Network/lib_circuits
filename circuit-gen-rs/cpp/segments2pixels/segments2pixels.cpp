...
Segments2Pixels<DrawableWhereT>::Segments2Pixels(
  ...
  if (nb_segments_ != drawables_.size() * nb_segs_per_digit) {
...
std::string Segments2Pixels<DrawableWhereT>::GenerateVerilog() const {
  ...
  // TODO
  verilog_buf += fmt::format("input [{:d}:0] s; // segments to display\n",
                             nb_segments_ - 1);
  verilog_buf +=
...
std::string Segments2Pixels<DrawableWhereT>::GetDefines() const {
  ...
  // HEIGHT
  // RNDSIZE
  verilog_defines.AddDefine("BITMAP_NB_SEGMENTS", nb_segments_);
  verilog_defines.AddDefine("WIDTH", width_);
  verilog_defines.AddDefine("HEIGHT", height_);
  if (has_watermark_) {
    verilog_defines.AddDefine("HAS_WATERMARK");
  }
  verilog_defines.AddDefine("RNDSIZE", rndsize_);

  return verilog_defines.GetDefinesVerilog();
}

template <typename DrawableWhereT>
void Segments2Pixels<DrawableWhereT>::GetConfig(uint32_t* nb_segments,
                                                uint32_t* rndsize) const {
